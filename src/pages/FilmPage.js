...
import { createToken } from "../app/themoviedbApi";
...
function FilmPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const response = await apiService.get(`/movie/${id}?api_key=${API_KEY}`);
    ...
    return (
        <Stack sx={{ p: 4 }}>
            <Breadcrumbs sx={{ marginBottom: 2 }} aria-label="breadcrumb">
                ...
                <Typography color="text.primary">FILM</Typography>
            </Breadcrumbs>
            {loading ? (
                <LoadingScreens />
            ) : error ? (
                    ...
                    >
                        <Grid container spacing={4}>
                            ...
                            <Grid item container xs={12} md={6} rowSpacing={0}>
                                ...
                                <Grid item xs={8}>
                                    <Typography sx={{ fontStyle: "italic" }}>{film["overview"]}</Typography>
                                </Grid>
                                <Grid item xs={4}>
                                    <Typography>IMDb:</Typography>
                                </Grid>
                                <Grid item xs={8}>
                                    <Typography sx={{ fontStyle: "italic" }}>{film["vote_average"]}</Typography>
                                </Grid>
                                <Grid item xs={4}>
                                    <Typography>Runtime:</Typography>
                                </Grid>
                                <Grid item xs={8}>
                                    <Typography sx={{ fontStyle: "italic" }}>{film["runtime"]}'</Typography>
                                </Grid>
                                <Grid item xs={4}>
                                ...
                                <Grid item xs={8}>
                                    ...
                                    >
                                        {film["genres"].map((item) => (
                                            <Chip color="primary" key={item["name"]} label={item["name"]} />
                                        ))}
                                    </Paper>
                                </Grid>
                                <Grid item xs={4}>
                                ...
                                <Grid item xs={8}>
                                    <Typography sx={{ fontStyle: "italic" }}>
                                        {film["production_companies"].map((item) => item["name"]).join(", ")}
                                    </Typography>
                                </Grid>
                                <Grid item xs={4}>
                                ...
                                <Grid item xs={8}>
                                    <Typography sx={{ fontStyle: "italic" }}>
                                        {film["production_countries"].map((item) => item["name"]).join(", ")}
                                    </Typography>
                                </Grid>
                                <Grid item xs={4}>
                                ...
                                <Grid item xs={8}>
                                    <Typography sx={{ fontStyle: "italic" }}>{film["release_date"]}</Typography>
                                </Grid>
                            </Grid>
                        </Grid>
                    </Stack>
